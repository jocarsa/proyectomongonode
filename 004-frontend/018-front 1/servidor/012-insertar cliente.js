...
app.post('/insertarCliente', async (req, res) => {
  ...
  if (!nombre || !apellidos) {
    return res.status(400).send('Nombre y apellidos son requeridos');
  }
  ...
  try {
    ...
    const result = await collection.insertOne({ nombre, apellidos });
    
    res.status(201).send({ mensaje: 'Cliente insertado con éxito', clienteId: result.insertedId });
  } catch (error) {
